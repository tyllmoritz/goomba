...
;----------------------------------------------------------------------------
	ldrb r2,gb_ime
	tst r2,#1		;no Halt if IRQ disabled.
	beq _noStop
...
;----------------------------------------------------------------------------
	...
	addne gb_a,gb_a,#0x06000000
	cmp r1,#0x0A000000
	addpl gb_a,gb_a,#0x06000000

	tst gb_flg,#PSR_C		;carry.
	addne r0,r0,#0x60000000
	cmp gb_a,#0xA0000000
	addhs r0,r0,#0x60000000

	adds gb_a,gb_a,r0
	and gb_flg,gb_flg,#PSR_C|PSR_n	;only keep C & n
	orrcs gb_flg,gb_flg,#PSR_C	;C
	orreq gb_flg,gb_flg,#PSR_Z	;Z
	fetch 4
doSUBconv
	tst gb_flg,#PSR_h		;half carry.
	addne r0,r0,#0x06000000
...
;----------------------------------------------------------------------------
	ldrsb r0,[gb_pc],#1
	ldr r1,gb_sp
	eor r2,r1,r0,lsl#16		;prepare for h check.
	adds r1,r1,r0,lsl#16
	str r1,gb_sp
	mrs gb_flg,cpsr          	;C
	and gb_flg,gb_flg,#PSR_C	;only keep C
	eor r2,r2,r1
	tst r2,#0x10000000		;h, correct.
...
;----------------------------------------------------------
	...
	moveq r2,#1
	streqb r2,gb_ic			;interrupt confirm (for HALT)
...
;----------------------------------------------------------------------------
	...
	fetch 8
;----------------------------------------------------------------------------
_CB46;		BIT x,(HL)	, actually CB-46,4E,56,5E,66,6E,76 & 7E
;----------------------------------------------------------------------------
	readmemHL
	mov r2,r0,lsl#16
	ldrb r0,[gb_pc,#-1]
...
;----------------------------------------------------------------------------
gb_reset	;called by loadcart (r0-r9 are free to use)
;----------------------------------------------------------------------------
	mov addy,lr
...
;	bpl nr0
	...
	mov gb_a,#0x01000000	;0x01=Normal GB/SGB, 0xFF=PocketGB/SGB2, 0x11=CGB/AGB.
	mov gb_bc,#0x00130000	;Breg->0=Normal,1=AGB
	...
	mov pc,addy
normalops
	DCD _xx,_xx,_xx,_xx,_xx,_xx,_xx,_xx,_xx
jmpops
	DCD _xx,_xx,_xx,_xx,_xx,_xx,_xx,_xx,_xx
opindex
...
op_table
  ...
  ;writemem_tbl
	...
	DCD IO_W	;$E000
   ;memmap_tbl
rommap	% 4*4		;$0000-7FFF (rommap only used for savestates)
	DCD 0		;$8000
	DCD 0		;$A000
	DCD 0		;$C000
	DCD 0		;$E000
...
cpustate
	...
	DCD 0 ;gb_sp:
	DCB 0 ;gb_ime:	(interuppt master enable)
	DCB 0 ;gb_ie:	(interuppt enable, adr $FFFF)
	DCB 0 ;gb_if:	(interuppt flags, adr $FF0F)
	DCB 0 ;gb_ic:	(interuppt confirm, HALT)
