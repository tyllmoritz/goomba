...
void *ewram_textstart;

int roms;//total number of roms
int selectedrom=0;
int ui_visible=0;
int border_visible=0;
int ui_x=0;
int ui_y=0;
#if POGOSHELL
...
#if CARTSRAM
	{
		u8* ewram_start=(u8*)ewram_textstart;
...
{
	int i,j,topline=0,toprow,romflags=0;
	u8 *p;
//	romheader *ri;
...
{
	if (!(ui_visible && border_visible))
	{
		REG_BLDCNT=0x01f7;				//darken bg0,bg1,bg2
	}
	else
	{
		REG_BLDCNT=0x01fB;				//darken bg0,bg1,bg3
	}
