...
extern char fatBuffer[BYTE_PER_READ];
...
-----------------------------------------------------------------*/
u32 MPCF_LastSector (void)
{
	return ( MP_REG_LBA1 + (MP_REG_LBA2 << 8) + (MP_REG_LBA3 << 16) );
}
#endif
...
FAT_FILE* FAT_fopen(const char* path, const char* mode)
{
	...
	if ( strpbrk(mode, "wW") != NULL ) // (ucase(mode[0]) == 'W')
	{
		if (dirEntry.name[0] == FILE_FREE)	// Create file if it doesn't exist
		{
			...
			dirEntry.cTime_ms = 0;
			dirEntry.cTime = 0;//getRTCtoFileTime();
			dirEntry.cDate = 0;//getRTCtoFileDate();
			dirEntry.aDate = 0;//getRTCtoFileDate();
			dirEntry.mTime = 0;//getRTCtoFileTime();
			dirEntry.mDate = 0;//getRTCtoFileDate();
		}
...
bool FAT_fclose (FAT_FILE* file)
{
	...
	if ((file != NULL) && (file->inUse == true))
	{
		if (file->write || file->append)
		{
			...
			((DIR_ENT*)globalBuffer)[file->dirEntOffset].mTime = 0;//getRTCtoFileTime();
			((DIR_ENT*)globalBuffer)[file->dirEntOffset].mDate = 0;//getRTCtoFileDate();
			((DIR_ENT*)globalBuffer)[file->dirEntOffset].aDate = 0;//getRTCtoFileDate();
